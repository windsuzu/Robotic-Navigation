...
class Particle:
    ...
    def compute_H(self, lx, ly):
        ...
        H = np.array([[delta[0]/np.sqrt(q), delta[1]/np.sqrt(q)],
                      [-delta[1]/q, delta[0]/q]])
        return H
    ...
    def update_landmark(self, z, lid):
        ...
        if lid not in self.landmarks:
            ...
            p = multi_normal(np.array(z).reshape(
                2, 1), np.array(z_hat).reshape(2, 1), Q)
        return p
...
def motion_model(pos, control, motion_noise=[0]*6):
    ...
    v_hat = v + find_var(motion_noise[0], motion_noise[1]) * np.random.randn()
    w_hat = w + find_var(motion_noise[2], motion_noise[3]) * np.random.randn()
    w_hat = normalize_angle(w_hat)
    g_hat = find_var(motion_noise[4], motion_noise[5]) * np.random.randn()
...
def multi_normal(x, mean, cov):
    ...
    return np.exp(-(err.T)/2 @ np.linalg.inv(cov) @ err) / (2 * np.pi * np.sqrt(np.linalg.det(cov)))
