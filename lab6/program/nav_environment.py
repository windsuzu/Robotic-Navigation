...
class NavigationEnv:
    ...
    def initialize(self):
        ...
        self.car = KinematicModel(
            d=12, wu=12, wv=4, car_w=18, car_f=14, car_r=10, dt=0.1)
        #self.car = KinematicModel()
        ...
        self.target_dist = np.sqrt(
            (self.car.x - self.target[0])**2 + (self.car.y - self.target[1])**2)
        target_orien = np.arctan2(
            self.target[1]-self.car.y, self.target[0]-self.car.x) - np.deg2rad(self.car.yaw)
        target_rel = [self.target_dist *
                      np.cos(target_orien), self.target_dist*np.sin(target_orien)]
    ...
    def step(self, action):
        ...
        # Orientation Reward
        orien = np.rad2deg(np.arctan2(
            self.target[1] - self.car.y, self.target[0] - self.car.x))
    ...
    def render(self, gui=True):
        ...
        cv2.circle(
            img_, (int(1*self.target[0]), int(1*self.target[1])), 10, (1.0, 0.5, 0.7), 3)
