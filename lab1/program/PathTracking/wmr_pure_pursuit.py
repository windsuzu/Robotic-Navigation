...
class PurePursuitControl:
    ...
    def feedback(self, state):
        ...
        # hint: (if you can not find a point(target) on the path which
        # distance between the path and model is as same as the Ld, you need
        ...
        # update alpha
        alpha = ((self.path[idx, 1]-y) /
                 (self.path[idx, 0]-x)) - np.deg2rad(yaw)

        # update next weight
        next_w = np.rad2deg(2*v*np.sin(alpha) / distance)
