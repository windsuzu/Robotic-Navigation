...
def path2(p1=80.0):
    ...
    diff2 = [(-np.sin(ix/p1)/(p1**2)*ix + np.cos(ix/p1) /
              p1 + np.cos(ix/p1)/p1)/4.0 for ix in cx]
    cyaw = [np.rad2deg(np.arctan2(d1, 1)) for d1 in diff1]
    ccurv = [np.abs(diff2[i])/np.power((1+diff1[i]**2), 3/2)
             for i in range(len(cx))]
    path = np.array([(cx[i], cy[i], cyaw[i], ccurv[i])
                     for i in range(len(cx))])
    return path
