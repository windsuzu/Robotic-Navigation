...
def _draw_rectangle(img, x, y, u, v, phi, color=(0, 0, 0), size=1):
    ...
    cv2.line(img, tuple(pts1.astype(np.int).tolist()),
             tuple(pts2.astype(np.int).tolist()), color, size)
    cv2.line(img, tuple(pts1.astype(np.int).tolist()),
             tuple(pts3.astype(np.int).tolist()), color, size)
    cv2.line(img, tuple(pts3.astype(np.int).tolist()),
             tuple(pts4.astype(np.int).tolist()), color, size)
    cv2.line(img, tuple(pts2.astype(np.int).tolist()),
             tuple(pts4.astype(np.int).tolist()), color, size)
    return img
...
class KinematicModel:
    ...
    def update(self):
        ...
        self.yaw += np.rad2deg(self.v / self.l *
                               np.tan(np.deg2rad(self.delta))) * self.dt
    ...
    def _compute_car_box(self):
        pts1 = _rot_pos(self.car_f, self.car_w/2, -self.yaw) + \
            np.array((self.x, self.y))
        pts2 = _rot_pos(self.car_f, -self.car_w/2, -self.yaw) + \
            np.array((self.x, self.y))
        pts3 = _rot_pos(-self.car_r, self.car_w/2, -self.yaw) + \
            np.array((self.x, self.y))
        pts4 = _rot_pos(-self.car_r, -self.car_w/2, -
                        self.yaw) + np.array((self.x, self.y))
        self.car_box = (pts1.astype(int), pts2.astype(int),
                        pts3.astype(int), pts4.astype(int))
    ...
    def render(self, img=np.ones((600, 600, 3))):
        ...
        for i in range(start, len(self.record)-1):
            ...
            cv2.line(img, (int(self.record[i][0]), int(self.record[i][1])), (int(
                self.record[i+1][0]), int(self.record[i+1][1])), color, 1)
        ...
        cv2.line(img, (int(self.x), int(self.y)),
                 (int(t1[0]), int(t1[1])), (0, 0, 1), 2)
        cv2.line(img, (int(t2[0]), int(t2[1])),
                 (int(t3[0]), int(t3[1])), (1, 0, 0), 2)
        ...
        # 4 Wheels
        img = _draw_rectangle(img, int(w1[0]), int(
            w1[1]), self.wu, self.wv, -self.yaw-self.delta)
        img = _draw_rectangle(img, int(w2[0]), int(
            w2[1]), self.wu, self.wv, -self.yaw-self.delta)
        img = _draw_rectangle(img, int(w3[0]), int(
            w3[1]), self.wu, self.wv, -self.yaw)
        img = _draw_rectangle(img, int(w4[0]), int(
            w4[1]), self.wu, self.wv, -self.yaw)
        # Axle
        img = cv2.line(img, tuple(w1.astype(np.int).tolist()),
                       tuple(w2.astype(np.int).tolist()), (0, 0, 0), 1)
        img = cv2.line(img, tuple(w3.astype(np.int).tolist()),
                       tuple(w4.astype(np.int).tolist()), (0, 0, 0), 1)
        return img
